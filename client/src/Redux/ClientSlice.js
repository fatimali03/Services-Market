...
export const companyServices = createAsyncThunk(
  "client/allServices",
  async (_, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem("token");
...
const clientSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(companyServices.fulfilled, (state, action) => {
      state.data = action.payload;
    });
    builder.addCase(companyServices.rejected, (state, action) => {
      state.error = action.payload;
    });
    // Get Service Info
