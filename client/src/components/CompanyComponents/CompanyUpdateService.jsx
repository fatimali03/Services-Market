...
import { showService, updateService } from "../../Redux/CompanySlice";
import Loading from '../Loading';
import { toast } from 'react-toastify';

export default function CompanyUpdateService() {
    ...
    useEffect(() => {
        tokenExists(token, navigate, dispatch).then(data => (data == false || JSON.parse(localStorage.getItem('userInfo')).role != "company" || JSON.parse(localStorage.getItem('userInfo'))._id != id) && navigate("/login"))
    }, [])
                ...
                dispatch(updateService({ serviceId, body })).unwrap().then(data => {
                    setTimeout(() => {
                        ...
                        if (data.status == 200) {
                            ...
                            navigate(`/dashboard/company/${id}/services/manage`)
    ...
    return (
        <>
            {loading && <Loading />}
            <div className="CompanyUpdateService">
                <div className="container">
                    <div className="section">
                        <HashLink className="go-back-button" to={`/dashboard/company/${id}/services/manage`}><button>Go Back</button></HashLink>
                        <div className="updateHeader">
                            Update Service
                        </div>
                        <form encType="multipart/form-data" onSubmit={e => handleSubmit(e)}>
                            <div className="form-section">
                                <label htmlFor="gig">Service Offered</label>
